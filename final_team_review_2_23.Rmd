...
# Executive Summary
-- summarizing our dataset and the problem
-- discussing the different methods we used 
-- don't need to mention the specific MSE's and RMSE's, just say which performed the best and worst
-- tie into the story - this method can be used to predict apartment prices in South Korea

As a team, we set out to develop a model to predict apartments prices in South Korea based on various features about the location and attributes of the particular apartment. Housing prices is an area of big concern all over the world given our ever growing population. South Korea, a country in which about 59.9% of all homes are apartments, is no exception. We will specifically be examining apartments in order to understand which factors contribute most to apartments prices, and to predict prices for apartments that we have not yet seen. For a family searching for a home in South Korea, this model can help them to udnerstand if they are overpaying. For example, if they know the location of the apartment and some basic information, such as the number of rooms, they can determine if they are being offered a fair price.
...
* supply_area:
* total_housefol_count_of_area_type
* room_count: the number of rooms in the apartment
* bathroom_count: the number of bathroms in the apartment
...
# we want to select random columns and set the seed to get consistent results
set.seed(810)
