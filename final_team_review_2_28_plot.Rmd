...
In the below model, we started with just 5000 trees. We then go on to perofrm feature engineering, and use cross validation to find the optimal number of trees.
```{r}
...
cat("RMSE train: ", sqrt(mean((y_train - yhat.boost.train)^2)))
```
...
cat(" RMSE test: ", sqrt(mean((y_test - yhat.boost)^2)))
```
...
yhat.boost.cv.train = predict(boost.price.cv, newdata = train_sample, n.trees = which.min(boost.price.cv$cv.error))

cat("Train RMSE: ", sqrt(mean((train_sample$transaction_real_price - yhat.boost.cv.train)^2)))
# predict on the test dataset using the recommended number of trees
predictions <- predict(boost.price.cv, newdata=dd_train[-train_sample_index,], n.trees=which.min(boost.price.cv$cv.error))
cat(" Test RMSE: ", sqrt(mean(tree.testy_sample - predictions)**2))
```
...
yhat.boost.train = predict(boost.price.cv, newdata = train_sample, n.trees = which.min(boost.price.cv$cv.error))
cat("RMSE train: ", sqrt(mean((train_sample$transaction_real_price - yhat.boost.train)^2)))
